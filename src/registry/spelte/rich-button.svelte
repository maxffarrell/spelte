<script lang="ts">
	...
	const textShadow = $derived(textShadowMap[color]);
</script>

{#if href}
	<a {href} class={combined} {...props}>
		<div class={cn('relative flex items-center gap-2', textShadow)}>
			{@render children()}
		</div>
	</a>
{:else}
	<button class={combined} {...props}>
		<div class={cn('relative flex items-center gap-2', textShadow)}>
			{@render children()}
		</div>
	</button>
