...
<Marquee class="flex py-4" duration={40}>
  ...
  {/each}
	...
	const tabTriggerClass =
		'rounded-md cursor-pointer data-active:bg-white px-2 text-center data-active:shadow-[0_0_0_1px_rgba(0,0,0,.08),_0px_2px_2px_rgba(0,0,0,.04)] dark:data-active:bg-[#0B0B09]';
	...
	function highlightSvelte(value: string) {
		...
		return escaped.replace(
			pattern,
			(match, tagStart, tagName, attrs = '', tagEnd, comment, stringLiteral, keyword) => {
				if (comment) {
				...
				const highlightedAttrs = attrs.replace(
					/([\w:-]+)(=)(&quot;[^&]*&quot;|\{[^}]*\})/g,
					(_match: string, name: string, equals: string, attrValue: string) =>
	...
	const chartLabels = chartData.map((_, i) => {
		const d = new Date(start + (i / (chartData.length - 1)) * (end - start));
...
<Tabs.Root bind:value={tab}>
	...
	<Tabs.Content value="preview" class="mt-2">
		<div
			class="flex w-full {fullBleedPreview ? 'p-0' : 'p-10'} overflow-hidden border rounded-sm not-prose preview min-h-64 justify-center md:min-h-80 h-full items-center"
		>
		{#if id === 'badge'}
			<div class="flex gap-2">
				<Badge>Default</Badge>
		...
		{:else if id === 'shimmer-text'}
			<ShimmerText class="font-medium">Thinking longer for a better answer</ShimmerText>
		{:else if id === 'highlighted-text'}
			<div class="text-2xl md:text-4xl font-medium tracking-[-.03em] flex items-center">
		...
		{:else if id === 'words-stagger'}
			<div class="flex items-center justify-center">
				<WordsStagger
					text="Spell UI is an open source collection of elegant, user friendly components that seamlessly integrate with frameworks and AI models."
					class="text-2xl md:text-3xl font-[550] tracking-tight max-w-[600px]"
				/>
			</div>
		{:else if id === 'special-text'}
			<SpecialText text="SPELL UI" class="text-xl" />
		{:else if id === 'randomized-text'}
			<div class="flex justify-center items-center min-h-[200px]">
				<div class="flex flex-col items-start gap-4 max-w-2xl px-4">
					<RandomizedText
						text="Introducing Spell UI"
						split="chars"
		...
		{:else if id === 'text-marquee'}
			<TextMarquee
				items={[marqueeDesign, marqueeMotion, marqueeComponents, marqueeInterfaces]}
			/>
		{:else if id === 'marquee'}
			<Marquee class="flex py-4" duration={40}>
				...
				>
					...
					<div class="flex justify-between items-end">
						<div>
							...
							<p class="text-sm font-medium tracking-tight font-mono tabular-nums">
								5367 4567 8901 2345
							</p>
						</div>
						<div class="text-left">
							<p class="text-xs text-muted-foreground">Exp.</p>
							<p class="text-sm font-medium font-mono tabular-nums">12/25</p>
						</div>
					</div>
				</TiltCard>
			</div>
		...
		{:else if id === 'qr-code'}
			<div class="w-[140px] rounded-lg p-2 shadow-[0_0_0_1px_rgba(0,0,0,.08),_0px_2px_2px_rgba(0,0,0,.04)] dark:border-input dark:border [&_svg]:w-full [&_svg]:h-auto">
		...
		<Tabs.Content value="code" class="not-prose mt-2 [&_pre]:my-0">
			{#if previewSource[id]}
				<figure data-rehype-pretty-code-figure class="my-0">
					<pre
						class="shiki overflow-x-auto rounded-sm border bg-background p-4 text-sm"
						data-language="svelte"
						style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#0f0f0f"
					><code data-language="svelte">{@html highlightSvelte(previewSource[id])}</code></pre>
				</figure>
			{:else}
